...
var getOutcomeMsg = function (playerPointsString, ComputerPointsString) {
  ...
  if (playerPoints > computerPoints) {
    outputMsg = "Player wins! :partying_face:";
  } else if (playerPoints < computerPoints) {
    outputMsg = "Computer wins :face_with_symbols_on_mouth:";
  } else {
    outputMsg = "It's a draw :grimacing:";
  }
  return outputMsg;
};
...
var main = function (input) {
  ...
  if (currentGameMode == "Deal cards") {
    ...
    if (checkBlackjack(playerCards[0], playerCards[1])) {
      myOutputValue = `${myOutputValue}<br><br>Huat ah! Player won with a blackjack! :sunglasses:`;
  ...
  if (currentGameMode == "Player action") {
    ...
    if (input == "Stand") {
      if (checkBlackjack(computerCards[0], computerCards[1])) {
        myOutputValue = `${myOutputValue}<br>Oh no! Computer won with a blackjack! :confounded:`;
  ...
  if (currentGameMode == "Computer action") {
    ...
    if (computerPointsFeedback == "below 17") {
      ...
      myOutputValue = `${myOutputValue}<br>
      Computer has ${computerPoints} points.<br><br>
      Computer busts, player wins! :partying_face:`;
      currentGameMode = "Deal cards";
    }
    return myOutputValue;
  }
...
// computer draw until it has a minimum of 17
